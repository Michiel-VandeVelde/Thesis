...
In this thesis, media behaviour data refers to records generated when a user interacts with media services, such as playing a video, pausing content, skipping a song, or entering a search query. A media behaviour profile refers to a structured representation derived from such data over time, including patterns such as preferred genres, typical moments of use, recurring interests, or cross-platform consumption habits.

This distinction is important because raw events and derived profiles are not the same. A single play, pause, or search action may seem harmless, but when many of these events are combined, they can reveal detailed information about a person. For example, long-term media behaviour may expose routines, interests, political views, emotional states, health-related concerns, or cultural identity. This is often referred to as the mosaic effect where separate pieces of data may appear insignificant, but together can form a sensitive and revealing profile.
...
The privacy risk is increased by the fact that most media platforms store this data centrally. Platforms often collect, analyse, and retain behavioural data within their own infrastructure, while users have limited insight into what is stored, how long it is kept, how it is used, or with whom it is shared. This creates an imbalance: platforms can build detailed profiles of users, while users have little control over those profiles.

Centralised storage also creates security and governance risks. Large behavioural datasets become attractive targets for breaches and secondary uses beyond the user’s original expectations. Once behavioural data has been used to train models or create derived profiles, it may be difficult to fully remove its influence, even if the original data is deleted. This creates both technical and governance challenges for rights such as data erasure Article 17 of the GDPR [8] and motivates the need for architectures where users have greater control over where their behavioural data is stored and how it is used.



### Towards a User-Centric Architecture: Core Principles

A user-centric media profile architecture challenges the traditional platform-controlled model. Instead of each platform storing and governing user data independently, the user maintains their data in a personal data store. Platforms may only access or contribute to this data with explicit user consent, for a clearly defined purpose, and within a specified time frame. This introduces several requirements that guide the design choices discussed in this thesis.

**Sovereignty** is a foundational principle in the European data landscape. EU policy-makers have increasingly framed the concentration of data and infrastructure in the hands of a small number of non-EU technology companies as a strategic concern, both for the European data economy and for the protection of fundamental rights [22]. In the context of personal data, sovereignty means that individuals retain effective control over the data they generate, not only formal ownership, but practical means to govern access, use, and movement. Without sovereignty as an explicit design constraint, user-centric systems risk collapsing into a weaker form of delegated control, in which users nominally possess their data but lack the interfaces, standards, and institutional support required to actually govern it [23].
...
**Compliance** in decentralised ecosystems cannot rely on a single governing authority. Instead, it is achieved through standardised rules, verification frameworks, and alignment with regulatory instruments such as the GDPR [8]. Initiatives such as Gaia-X propose federated mechanisms, including Gaia-X Digital Clearing Houses, that act as trusted intermediaries for data exchange across organisational and jurisdictional boundaries [5]. The underlying idea is that data can remain under the control of its owner while still being exchangeable under verifiable rules, which distinguishes federated governance from both fully centralised and fully peer-to-peer models [24].
...
For deployment within the European context, the Community Solid Server (CSS), developed by imec-IDLab, offers a ready and open-source implementation of the Solid specifications [15]. CSS is modular by design, with configurable components for storage backends, authentication flows, and access control mechanisms. This makes it useful for research prototypes and experimental deployments where different architectural choices need to be tested. For this thesis, its main value lies in its flexibility: components such as policy enforcement, aggregation, or custom access-control behaviour can be explored through extensions or configuration, without requiring changes to the whole server architecture.
...
However, identity also creates privacy challenges. If the same WebID is used across many services, it may become a point of correlation. Several services could recognise that they are interacting with the same user. For this reason, privacy-preserving deployments may benefit from different WebIDs for different contexts, such as personal media use, professional activity, or research participation.
...
Trust is essential in a decentralised architecture. Since data is no longer controlled by one central platform, the system needs mechanisms that make access and usage transparent. Authentication, access control, policy enforcement, and logging all contribute to this trust model.

Audit logs can record who accessed which data, when the access took place, and for what stated purpose. These logs may be stored in the user’s pod, giving the user direct insight into how their data has been accessed. This is different from traditional platform-controlled systems, where logs are usually kept internally by the platform and are not easily visible to the user.

Audit logs can also be structured as an append-only event stream, for example using LDES. If logs are signed, timestamped, or linked cryptographically, they can become more resistant to tampering. However, this property should not be assumed automatically. It depends on the specific logging mechanism used.

Auditability supports accountability. If a service accesses data outside the agreed conditions, the user or regulator needs evidence of what happened. Transparent access records can therefore support compliance with principles such as transparency, accountability, and data protection by design under the GDPR [8].

Together, these technologies address different layers of the problem. Solid provides user-controlled storage, Solid-OIDC and WebID provide identity, WAC and ACP provide access control, ODRL supports policy expression, LDES supports incremental event processing, and Gaia-X/Dataspace initiatives provide a broader governance context. However, no single component fully solves the problem of cross-service media behaviour profiling. The architectural challenge is therefore to combine these components into a coherent system that balances user control, interoperability, usability, performance, and accountability.

### Dataspace Integration (Eventual interoperability)

While the architecture described in this thesis focuses on individual data sovereignty, it is designed to be extensible toward broader European data space initiatives.
