...
As illustrated in figure 3, different actors interact with different parts of the pod using different verbs. A media service may POST a new interaction event or DELETE a retracted one, while an aggregation agent uses GET to read events and PUT to write back an updated profile. A recommendation service is restricted to GET on the aggregated profile container only requests to raw event containers return 403 Forbidden. This per-resource access control is enforced by WAC or ACP policies attached directly to the pod, not by the applications themselves.
...
While the architecture described in this thesis focuses on individual data sovereignty, it is designed to be extensible toward broader European data space initiatives. This extensibility can be understood through the idea of eventual interoperability. Eventual interoperability means that a system does not need to fully align with every external vocabulary, standard, or data space from the beginning. Instead, it should use explicit semantics, stable identifiers, and well-documented data structures so that alignment with other systems can be introduced later when concrete use cases require it [22].
...
This vision aligns closely with the European Strategy for Data, which promotes the creation of common European data spaces based on interoperability, trust, and compliance with European rules and values [19].
...
Initiatives such as Gaia-X contribute to this broader vision by addressing data sharing at the level of federated ecosystems rather than individual applications or single platforms. Gaia-X is not intended to be one central cloud service. Instead, it defines a framework that links multiple cloud providers, service providers, data providers, and data consumers through shared specifications, rules, policies, and verification mechanisms. Its goal is to create a trusted environment in which data can be made available and exchanged while participants retain sovereignty over their data [5]. This makes Gaia-X relevant to the architecture proposed in this thesis because it shows how decentralised or federated systems can still require common governance structures. Where Solid focuses mainly on user-controlled personal data storage and access through pods, Gaia-X focuses on the wider organisational layer: transparency, compliance, interoperability, data sovereignty, service descriptions, identity and access management, and mechanisms for reducing dependency on a single provider. In this context, future integration with frameworks such as the Eclipse Dataspace Protocol [20] and CEN/CENELEC trusted data transaction standards would represent a natural next step toward compatibility with the evolving European data ecosystem. Eventual interoperability supports this by allowing the architecture to remain useful in its media-specific context now, while avoiding design choices that would make later integration with broader data spaces difficult.

### Stakeholder Considerations

To situate the proposed architecture within the Flemish media landscape, a semi-structured expert interview was conducted with a practitioner from the Flemish media sector. The interviewee is a Programme and Communications Director at MediaNet Vlaanderen, with direct operational experience in media data governance and involvement in the Solid4media initiative, an ongoing industry project that explores the application of Solid technology to media data management and closely aligns with the scope of this thesis.
