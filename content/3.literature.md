...
WAC is relatively simple, which makes it easy to understand and implement. However, this simplicity also limits what it can express. For example, WAC is not well suited for more specific conditions such as “only allow access when the request comes from a specific application” or “only allow access when the user is authenticated through a trusted identity provider.”
...
To situate the proposed architecture within the Flemish media landscape, consultations were conducted with domain experts from the Solid4media initiative, an ongoing industry project that directly explores the application of Solid technology to media data management and closely aligns with the scope of this thesis. The insights below reflect perspectives from practitioners with organisational experience in the Flemish media sector, including a Programme and Communications Director at MediaNet Vlaanderen.
