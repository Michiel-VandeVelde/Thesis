...
While the architecture described in this thesis focuses on individual data sovereignty, it is designed to be extensible toward broader European data space initiatives. This extensibility can be understood through the idea of eventual interoperability. Eventual interoperability means that a system does not need to fully align with every external vocabulary, standard, or data space from the beginning. Instead, it should use explicit semantics, stable identifiers, and well-documented data structures so that alignment with other systems can be introduced later when concrete use cases require it [26].
