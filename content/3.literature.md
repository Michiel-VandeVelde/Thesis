...
<!-- Pieter: For citations, add the bibtex reference in references.bib and use [text you want to point at for the citation](cite:cites label) → you can also use other terms than just “cite” from the cito ontology: https://sparontologies.github.io/cito/current/cito.html (like: “refutes”) if you want to have more richer citations-->
...
<!-- Pieter: what is “the privacy risk”? Is it the risk above? Maybe refer to “this privacy risk instead then? -->
The privacy risk is increased by the fact that most media platforms store this data centrally. Platforms often collect, analyse, and retain behavioural data within their own infrastructure, while users have limited insight into what is stored, how long it is kept, how it is used, or with whom it is shared. This creates an imbalance: platforms can build detailed profiles of users, while users have little control over those profiles. <!-- Pieter: You will probably need to explain control here-->

<!-- Pieter: one could argue the oposite as well: due to the fact that your data is here and there, it becomes difficult to secure all these different locations -->
Centralised storage also creates security and governance risks. Large behavioural datasets become attractive targets for breaches, legal requests, and secondary uses beyond the user’s original expectations. Once behavioural data has been used to train models or create derived profiles, it may be difficult to fully remove its influence, even if the original data is deleted. This creates tension with rights such as data erasure under the GDPR [8] and motivates the need for architectures where users have greater control over where their behavioural data is stored and how it is used.
...
<!-- PC: Compliance cannot rely → you often use these kind of constructs (which is fine if you do it once for stylistic reasons) - but ”compliance” is not something that can “rely” and it makes your text weaker here: who cannot rely on a single governing authority here? And why not? It depends on the use case, right? E.g., I’m quite confident I trust that the government knows whether I’m registered as a Belgian or not as an authority. -->
**Compliance** in decentralised ecosystems cannot rely on a single governing authority to enforce the rules. Instead, it must be achieved through standardised rules, verification frameworks, and alignment with regulatory requirements such as the GDPR [8]. Initiatives such as Gaia-X introduce mechanisms like Digital Clearing Houses to ensure that data exchanges remain trusted and auditable across organisational boundaries [5]. At their core,
GXDCHs serve as trusted intermediaries within the Gaia-X community, facilitating the seamless and secure flow of data between different entities. The concept of 'federated' is integral to their operation. In a federated model, data remains decentralized and under the control of its owners, while still being accessible to authorized users across different organisations and jurisdictions. This approach allows for a highly secure, scalable, and interoperable environment where data providers and consumers can interact without compromising data sovereignty or compliance. - https://gaia-x.eu/services-deliverables/digital-clearing-house/

<!-- PC: *all* data access and usage? Or should you be able to turn it on when you have a use case that requires it? -->
...
<!-- PC: when using acronyms, first write them in full: LDES → Linked Data Event Streams (LDES)-->
...
<!-- PC: instead of writing it’s essential, write the reason why you believe the reader should infer it’s indeed essential. I dislike the construct of “X is essential / important / crucial / significant / ...” because it’s persuasive and not informative: you try to ensure the reader is on your side without giving argumentation, and the consequence is you’ll loose the reader instead. I would just remove the first sentence and the paragraph wil lstill work. -->
...
<!-- PC: you also don’t explain eventual interop really here? -->
...
<!-- PC: Limitations of what? Of the full SOTA? Aren’t these challenges that are always worked on? “Performance” is never going to be fully solved, it’s always a thing you’ll need to take into account no?-->
