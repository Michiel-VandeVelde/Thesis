...
These five building blocks are not independent. They form a chain: when a media service records a user interaction, it authenticates using Solid-OIDC and the user's WebID, then writes the event to the appropriate container in the user's pod. The policy engine checks whether this write matches the user's stated consent before it is permitted. Periodically, the aggregation agent reads permitted data from multiple service containers, computes a cross-service profile, and stores it back in the pod. Every access by the service, the policy engine, and the aggregation agent is recorded in an audit log the user can inspect. This chain connects identity, storage, policy, aggregation, and auditability into a single governed data flow that is transparent at every step.
