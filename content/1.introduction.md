...
In practice, however, many SPARQL dialects exist each introducing their own extensions or limitations.
...
which will deliver more rapid successions of SPARQL once SPARQL 1.2 is finalized.
<!-- -->
...
3. **Tooling maintainability**: Tools that support multiple SPARQL versions typically do so by maintaining multiple software versions,
...
The federated user query (blue) uses SPARQL 1.2 and uses the [non-standard builtin function ADJUST](cite:cites sparql-adjust).
The query targets four SPARQL endpoints, all with different SPARQL version, [RDF profile](cite:cites rdf-1-2), and set of language extension support.
Although the query engine can discover the version, profile, and supported features of each endpoint,
this does not guarantee it can resolve these discrepancies.
Traqula provides a maintainable way to support multiple SPARQL dialects,
allowing it to be the bridge between them. 
</figcaption>
</figure>

<!-- federation in heterogeneous ecosystem is a problem - and RDFs distributed nature highlights this problem -->
The query evaluation problem within a heterogeneous SPARQL ecosystem becomes especially visible in federated query execution.
...
SPARQL reflects this distributed nature through support for [federated queries](cite:cites spec:sparqlfed),
where a single query may involve multiple endpoints, each with its own capabilities, limitations, and language features.

In such a setting, a user query written in one SPARQL version/dialect may not be executable on all federation targets.
...
[](#federation) illustrates this scenario: the query is written in SPARQL 1.2 and uses the non-standard [ADJUST function](cite:cites sparql-adjust),
...
it does not solve the underlying language mismatch.
To solve the mismatch there is a need for parsing and generating software that can handle various SPARQL dialects,
and bridge the dialects through effective transformations.
...
we proposed the concept of a modular SPARQL parser to address the growing heterogeneity of SPARQL dialects using
builder-based [inversion of control](cite:cites fowler-inversion-of-control).
That work focused on the design and a prototype implementation demonstrating parser composability,
...
In this paper, we extend that foundation with a complete implementation supporting round-tripping
...
<!-- Wrap up -->
We present Traqula, a modular SPARQL toolkit implementing these ideas.
Traqula has already been integrated into the Comunica query engine, demonstrating its maturity and practical applicability. 
Its modular and composable architecture, enables researchers and practitioners to:
<!-- -->
1. Experiment with grammar changes, including adding, modifying, or removing rules;
2. tackle the complexities of federated querying in a heterogeneous SPARQL ecosystem; and
3. lay the foundation for future query formatting and rewriting tools based on round-tripping.
<!-- -->
Traqula is implemented in TypeScript and is available as open-source software under the MIT license on GitHub and npm,
providing the community a robust and flexible resource for building tooling and engines in a heterogeneous SPARQL environment.


<!-- Structure explained -->
The remainder of this paper is structured as follows.
[](#requirements) details the requirements, [](#sota) reviews related work,
