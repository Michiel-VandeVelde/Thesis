...
<td>Partially</td>
<td>Both slices use JSON-LD with shared schema.org vocabulary. The cross-service profile uses <code>schema:sameAs</code> to link music and video creator identities confirmed by the user. For well-known creators, links are resolved automatically via Wikidata SPARQL and stored with <code>schema:isBasedOn</code> provenance. The aggregation processor derives unified interaction counts across both slices.</td>
