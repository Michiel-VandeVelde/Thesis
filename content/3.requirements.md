...
1. **Modularity**: <span class="comment" data-author="RT">This description does not seem like a real requirement, but more a way to achieve a requirement. I suspect the description to go more about the flexibility needs.</span> allowing you to compose your system out of small components and allowing you to change those components. 
...
3. **Web-based**: harnessing web technologies to maximize relevance and enable wide adoption. <span class="comment" data-author="RT">Additionally, I'd be a bit more specific here, and mention that it should run in the web browser, and outside.</span>
4. **Generic core**: <span class="comment" data-author="RT">Language-agnostic?</span> by providing a generic core, Traqula facilitates the support of many query languages.
...
Since migration from RDF 1.1 to RDF 1.2 is not trivial, <span class="comment" data-author="RT">I would start the sentence with the following part, and then say migration is not trivial due to that.</span> introducing the concept of recursive triples in the form of triple terms [](cite:cites rdf-1-2),
we expect that migration will require careful consideration <span class="comment" data-author="RT">Can you be more precise than "careful consideration"?</span> by many, slowing down adoption.
...
but is still supported by [some engines](cite:cites oxigraph-adjust)<span class="comment" data-author="RT">I remember making a comment previously about a better reference. This comment still holds.</span>.
...
<span class="comment" data-author="RT">We could add a requirement on "low barrier to entry for developers", which would allows us to land on JS/TS. (we could reference this recent GitHub post on TS being the most popular language.)</span>
