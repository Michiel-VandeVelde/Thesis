...
One limitation of this approach is that creator identity links are stored per user. Two users listening to the same artist on SoundCloud and watching the same channel on YouTube would each need to confirm the link independently. For well-known public creators this is unnecessary duplication: Shared knowledge graphs such as MusicBrainz and Wikidata already contain public identifiers and external links that can support creator identity resolution. A production deployment could address this by defaulting to public knowledge graph identity links for known creators and reserving user-confirmed links for cases where no public match exists. The distinction also clarifies what genuinely belongs in the pod: claims about public entities are better delegated to shared open data, while claims about the user's own cross-platform identity remain personal and user-sovereign.
