...
<figcaption markdown="block">
Visualisation of the indirection system.
The 'iri' implementation call an implementation 'iriFull' and 'prefixedIri',
once the iri function is evaluated, it will look into the RuleMap to call the linked implementation at that time. 
The visualisation also shows how calling the 'iri' implementation (done through the task builder)
means that the task builder instantiates the blue, task specific, function;
and afterward calls the red, context dependent function.
</figcaption>
</figure>
