...
For example, if a user only requires SPARQL 1.1 parsing, they can depend solely on _'@traqula/parser-sparql-1-1'_,
...
2. continue traversal into descendants;
...
[a catalogue of known existing transformations powered by Traqula](https://github.com/comunica/traqula/blob/main/docs/transformation-catalogue.md){:.mandatory},
thereby also demonstrating how Traqula can be used for traditional query optimizing or rewriting.

<figure id="transformer">
<pre style="overflow: auto"><code class="language-typescript" style="background: unset; font-size: 1em">const transformed = new TransformerTyped&lt;Sparql11Nodes&gt;()
...
<br class="printonly">
`SELECT * { { Select * { ?s ex:a ?o } } . ?s ex:b ?o . } LIMIT 20`
<br class="printonly">
would result in rewritten query
<br class="printonly">
`SELECT DISTINCT * { { Select * { ?s ex:a ?o } } . ?s ex:b ?o . } LIMIT 20`
.
</figcaption>
</figure>
