...
In this section we investigate the execution time performance of Traqula parsing,
comparing different modes available in Traqula,
comparing it with [SPARQL.JS](cite:cites sparqljs)/[SPARQLAlgebra.JS](cite:cites sparqlalgebrajs),
and providing a warning on parser reuse.
The measurements were executed on a computer running Fedora Linux 43 (Workstation Edition), on a 'Intel® Core™ Ultra 7 165U × 14' processor, having 16 GiB of RAM.
The execution times shown are the times required to parse 199 SPARQL 1.1 queries into the mentioned targets.


[](#traqula-bench) compares the execution time of Traqula using varous targets and parser capabilities.
The figure shows that patching a modular SPARQL 1.1 parser to support SPARQL 1.2 increases the execution time slightly, yet statistically significant (p < 0.05),
...
namely 16.5% <span class="todo">(6.27/5.38)</span>, and 17.5% <span class="todo">(6.71/5.71)</span> in the case of SPARQL 1.1 and SPARQL 1.2 parsing respectively, both significantly different (p < 0.05).
Similarly, parsing into algebra, meaning you parse into AST and perform some additional transformation increases the mean execution time further, measuring an increase of
58.7% <span class="todo">(8.54/5.38)</span> and 63.4% <span class="todo">(9.33/5.71)</span> for 1.1 and 1.2 respectively, which is also significant (p < 0.05).
...
This difference can be mostly devoted to the chosen system to construct the parser, meaning Chevrotain is just really fast.
...
<span class="todo">Maybe we should swap these graphs and see the first fig as a "zoom in" of the second one"</span>
...
<figcaption markdown="block">
Bargraph visualising the execution time in ms for parsing a collection of 199 queries into
<!-- -->
1. an AST,
2. an AST with source tracking (Traqula only), and
3. sparql algebra,
using both Traqula and SPARQL.JS/ SPARQLAlgebra.js.
We run the comparison using 1000 iterations on a SPARQL 1.1 parser and a SPARQL 1.2 parser that is created as a patch ontop of the 1.1 parser.
Note that the SPARQL.JS parser does not support SPARQL 1.2.
</figcaption>
</figure>
