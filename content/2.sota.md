...
1. **Lexical analysis** (or _scanning_) performed by a _lexer_: reading a source stream of characters and identifying non-overlapping sequences called _lexemes_.
   The description of what type of lexemes (called token type) should be identified is often done through a Regular Expression.
...
3. **Semantic analysis**: after having generated the tree, you check the non-structural constraints.
   ...
   In the context of parsing programming languages, a popular example of semantic analysis is type checking.

It should be noted that these three steps are conceptually distinct but are not necessarily executed distinctly.<span class="comment" data-author="RT">Do you mean "sequential" instead of "distinct"?</span>
...
1. **Generated**: Code generation tools come with their Domain Specific Language (DSL)
   that will typically share similarities with Extended Backus–Naur Form (EBNF) and some Regular Expression (regex/ regexp) <span class="comment" data-author="RT">Why do you mention regex/regexp like this? Just regular expression seems sufficient to me.</span> dialect.
...
2. **Hand-built**: Code generation can come with optimization limitations, when your grammar allows for optimizations not taken by the code generator.
   Just like writing assembly yourself<span class="comment" data-author="RT">I'd drop the assembly part. People with our background will understand, but many readers may not.</span>, writing a handwritten parser is powerful but very challenging to get right,
...
We will go on to argue <span class="comment" data-author="RT">Can you reference the section in which this is argued?</span> that parser-building toolkits provide a nice middle ground between both generated parsers,
and hand-built parsers while still providing excellent execution times according to the performed benchmark
<span class="todo">add footnote [https://chevrotain.io/performance/](https://chevrotain.io/performance/)</span>.

### Existing SPARQL/SQL parsers

[Previous work](cite:cites modular-parsing) which introduced <span class="comment" data-author="RT">You can use the "we"-form here.</span> the concept of modular parsing for SPARQL, contained a
...
<span class="comment" data-author="RT">Can we mention what type of parser each software package is/uses as a separate column?</span>
...
like through [ANTLR's grammar imports](https://github.com/antlr/antlr4/blob/857fb46e781ce9c40249b9f0156c67051cec12c1/doc/grammars.md#grammar-imports), they do so on parser granularity, and not rule granularity.
Specifically, parsers can be extended similar to an OOP <span class="comment" data-author="RT">First time OOP is used? Not explained yet.</span> class extension,
but rules cannot be deleted, nor can a patched rule still reference the original implementation.
Furthermore, all parser builders require a compile step, and most parser builders only provide a CST <span class="comment" data-author="RT">Same</span>,
necessitating a tree walk to create the AST <span class="comment" data-author="RT">Same(?)</span>, splitting the task of parsing to an AST in two.

### AST structures (round-tripping)

<span class="comment" data-author="RT">This section may contain a bit too much details for related work. Some things can be moved later.</span>
...
which allows you to write one version of JavaScript and compile/ transpile it to another version, empowering you to write new JavaScript and execute it on old environments. 
