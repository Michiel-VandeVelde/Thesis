...
It should be noted that these three steps are conceptually distinct but are not necessarily implemented distinctly nor executed sequentially.
...
In the next section we go on to argue that parser-building toolkits provide a nice middle ground between both generated parsers,
...
In [previous work](cite:cites modular-parsing) we introduced the concept of modular parsing for SPARQL, contained a
...
Specifically, parsers can be extended similar to an Object-Oriented Programming (OOP) class extension,
but rules cannot be deleted, nor can a patched rule still reference the original implementation.
Furthermore, all parser builders require a compile step, and most parser builders only provide a CST,
